...
RUN yum update -y && \
    ...
    scl enable rh-ruby23 "gem install --no-document  fluent-plugin-kubernetes_metadata_filter fluent-plugin-forest fluent-plugin-systemd fluent-plugin-secure-forward fluent-plugin-grep fluent-plugin-flatten-hash fluent-plugin-rewrite-tag-filter fluent-plugin-burrow" && \
